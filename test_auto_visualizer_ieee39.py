"""
Test script for Auto-Layout Power System Visualizer - IEEE 39-Bus System

Generates automatic layout visualizations of the IEEE 39-bus (New England)
system using impedance-based positioning (no hardcoded coordinates).

The IEEE 39-bus system includes:
- 10 generators (GENROU)
- 10 governors (TGOV1)
- 39 buses with transformers and transmission lines
"""
...
def run_auto_visualization(spacing_factor=2.0, node_size_factor=1.0,
    ...
    Args:
        spacing_factor: Controls componentspread around buses (default=2.0)
                       - 1.0 = default spacing (may have overlaps due to system size)
                       - 1.5 = good for moderate systems
                       - 2.0 = recommended for IEEE 39-bus (prevents overlaps)
                       - 2.5 = maximum spread for extremely crowded diagrams
        node_size_factor: Controls visual size of components (default=1.0)
                         - 1.0 = default size
                         - 0.7 = recommended for large systems
                         - 0.5 = minimum readable size
    """
    JSON_PATH = 'test_cases/ieee39bus/ieee39_system.json'

    print("\n" + "="*70)
    print("   AUTO-LAYOUT IEEE 39-BUS (NEW ENGLAND) SYSTEM VISUALIZER")
    ...
    try:
        ...
        print("-"*50)
        viz.draw("outputs/ieee39_auto_layout_hierarchical.svg", 
        ...
        print("-"*50)
        viz.draw("outputs/ieee39_auto_layout_force_directed.svg", 
        ...
        print("  - outputs/ieee39_auto_layout_hierarchical.svg")
        print("  - outputs/ieee39_auto_layout_force_directed.svg")
        print("  - outputs/ieee39_electrical_distance.svg")
...
if __name__ == "__main__":
    ...
    #   - 1.0 = default spacing (likely has overlaps in this large system)
    #   - 1.5 = moderate spacing
    #   - 2.0 = recommended for IEEE 39-bus (prevents most overlaps)
    #   - 2.5 = maximum spread (for very crowded areas)
    
    # NODE_SIZE_FACTOR: Controls the visual size of components
    #   - 1.0 = default size
    #   - 0.7 = recommended for large systems like IEEE 39-bus
    #   - 0.5 = minimum readable size
    #   - USE SMALLER VALUES to reduce overlaps without spreading the entire graph
    
    SPACING_FACTOR = 1.0  # <-- Adjust component positioning spread
    NODE_SIZE_FACTOR = 1  # <-- Adjust component visual sizes
